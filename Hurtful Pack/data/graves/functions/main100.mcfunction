...
execute if score $generate_trees$ g.usebonemeal matches 1.. as @a at @s unless entity @e[type=minecraft:armor_stand,tag=g.sapling_drop,distance=..280] unless entity @e[type=minecraft:area_effect_cloud,tag=g.notree,distance=..280] run function graves:tree_generator/revoke
