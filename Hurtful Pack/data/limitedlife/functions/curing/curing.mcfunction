...
execute if score @s ll.cooking matches 2100.. unless entity @e[type=minecraft:bat,distance=..48,tag=ll.bat,limit=1] unless entity @p[distance=..48,tag=ll.bat] at @e[type=minecraft:end_crystal,distance=..0.05,limit=1] run summon minecraft:tnt ~ ~ ~
...
execute unless entity @e[type=minecraft:end_crystal,distance=..0.05,tag=!ll.tired] run function limitedlife:curing/unmetconditions
#make sure there is only one ll.healingredient in the cauldron
execute at @s if entity @e[type=minecraft:item,distance=0.00000000000000000000001..0.5,tag=ll.healingredient] align xyz positioned ~0.5 ~ ~0.5 as @e[type=minecraft:item,distance=..0.5,tag=ll.healingredient,limit=1] run function limitedlife:curing/unmetconditions
