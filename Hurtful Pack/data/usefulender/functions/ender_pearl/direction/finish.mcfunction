...
execute at @s at @e[tag=ue.motion,distance=..12,limit=1,sort=nearest] run tp @e[tag=ue.motion,distance=..12,limit=1,sort=nearest] ~ ~ ~ facing entity @s
scoreboard players operation @s ue.pearlY = $distance$ ue.pearlthrow
execute as @e[tag=ue.motion,distance=..12,limit=1,sort=nearest] store result score @s ue.pearlZ run data get entity @s Rotation[1]
execute at @s at @e[tag=ue.motion,distance=..12,limit=1,sort=nearest] run function usefulender:ender_pearl/direction/check
