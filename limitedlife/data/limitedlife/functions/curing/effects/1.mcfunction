#summoning bat
execute if score @s ll.cooking matches 2000 at @e[type=minecraft:end_crystal,distance=..0.05,limit=1] run summon minecraft:bat ~ ~ ~ {DeathLootTable:"minecraft:empty",ActiveEffects:[{Id:14b,Amplifier:0b,Duration:100000,ShowParticles:0b}],Tags:["ll.bat"],Silent:1b,Invulnerable:1b}
...
execute if score @s ll.cooking matches 2020 run advancement revoke @a only limitedlife:catch_beam
#summoning endermite
