...
execute as @e[tag=sp.sapphire_glass,tag=sp.sapphire_glasssetup,type=minecraft:area_effect_cloud,limit=1,sort=nearest] at @s at @e[tag=sp.T.glass,type=minecraft:leash_knot,sort=nearest,limit=1] run tp @s ~ ~-300 ~
#end loop
