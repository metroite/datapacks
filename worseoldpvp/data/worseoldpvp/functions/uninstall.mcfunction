...
effect give @s[nbt={SelectedItem:{tag:{wopcooldown:0b}}}] minecraft:weakness 1 100 true
title @s[nbt={SelectedItem:{tag:{wopcooldown:0b}}}] actionbar ["",{"text":"worseoldpvp:","color":"green","clickEvent":{"action":"open_url","value":"https://github.com/Metroite/datapacks/tree/master/worseoldpvp"},"hoverEvent":{"action":"show_text","value":"reverts the combat system to the worse one back in 1.8 (drop your tools once to update them)"}},{"text":" drop your selected item once, its too fast, the datapack has been uninstalled!"}]
execute at @a as @e[type=minecraft:item,distance=..4,nbt=!{Item:{tag:{wopcooldown:1b}}}] run function worseoldpvp:cooldown/yes
